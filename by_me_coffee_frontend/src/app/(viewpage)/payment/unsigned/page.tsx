...
import { getProfile, sendDonationUnsigned } from "@/utils/request";
...
export default function PaymentPage() {
  ...
  const socialurl = searchParams.get("socialurl");
  const router = useRouter();
  const [succes, setSucces] = useState(false);
  const [profile, setProfile] = useState<Profile>();
  const [loading, setLoading] = useState(false);
  const sendDon = async () => {
    if (message && amount && socialurl) {
      setLoading(true);
      await sendDonationUnsigned(
        {
    ...
    <div className="w-scree h-screen flex items-center justify-center">
      {!succes ? (
        ...
        <div className="flex flex-col gap-8 items-center">
          ...
          {!loading && (
            <div className="flex flex-col gap-3 border p-6 rounded-lg">
              <div className="flex items-center text-lg ">
                <img
                  className="rounded-full h-8 w-8"
                  src={profile?.avatarImage ? profile.avatarImage : ""}
                  alt="avatar"
                />
